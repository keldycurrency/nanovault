...
export class RaiPipe implements PipeTransform {
  ...
  transform(value: any, args?: any): any {
    ...
    switch (denomination.toLowerCase()) {
      default:
      case 'kld': return `${(value / this.mrai).toFixed(6)}${!hideText ? ' KLD': ''}`;
      case 'mnano':
        ...
        if (hasRawValue) {
          ...
          return `${this.toFixed(newVal, this.precision)}${!hideText ? ' KLD': ''}`;
        } else {
          return `${(value / this.mrai).toFixed(6)}${!hideText ? ' KLD': ''}`;
        }
