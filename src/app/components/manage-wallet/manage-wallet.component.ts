...
export class ManageWalletComponent implements OnInit {
  ...
  exportAddressBookToFile() {
    if (this.walletService.walletIsLocked()) return this.notifications.sendWarning(`Wallet must be unlocked`);
    const fileName = `KeldyVault-AddressBook.json`;
  ...
  exportToFile() {
    ...
    const fileName = `KeldyVault-Wallet.json`;
    ...
    reader.onload = (event) => {
      ...
      try {
        ...
        if (!importData.length || !importData[0].account) {
          return this.notifications.sendError(`Bad import data, make sure you selected a KeldyVault Address Book export`)
        }
